...
func (this LoadMapping_CronJob) Run() {
	...
	EsMappingSender(next_day_index, this.mappings)
}
...
func EsMappingsSetup(base_url string, mappings string) {

	log.Println("Setup Mappings For Index:", base_url)
	log.Println("mappings is ", mappings)

	err := EsMappingSender(base_url, mappings)
	if err != nil {
		log.Println("EsMappingSender err:", err)
	}
