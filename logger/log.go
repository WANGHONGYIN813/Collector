...
func (this *LocalLogger) writeToLoggers(when time.Time, msg *loginfo, level int) {
	for _, l := range this.outputs {
		...
		msgStr := when.Format(this.timeFormat) + " [" + msg.Level + "] " + msg.Content
...
// param 是log配置文件名
func SetLogger(param string, filename_prefix string) error {
	if 0 == len(param) {
		//默认只输出到控制台
	...
	if conf.File != nil {
		...
		if filename_prefix != "" {
			...
			file_path := filepath.Dir(conf.File.Filename)

			purefilename := filepath.Base(conf.File.Filename)

			conf.File.Filename = file_path + "/" + filename_prefix + "-" + purefilename

			fmt.Println("Log Filename :", conf.File.Filename)
		}
