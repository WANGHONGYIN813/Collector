...
type fileLogger struct {
	...
	Starttrunc bool   `json:"starttrunc"`
...
var Max_File_Suffix_Num int = 10000
...
func test_path_exists(path string) bool {
	_, err := os.Stat(path)
	if err != nil {
		if os.IsExist(err) {
			return true
...
func (f *fileLogger) Init(jsonConfig string) error {
	...
	file_path := filepath.Dir(f.Filename)
	if test_path_exists(file_path) == false {
		err := os.MkdirAll(file_path, os.ModePerm)
		if err != nil {
			fmt.Printf("Fail to create Log Directory : %s , err: %s\n", file_path, err)
		} else {
			fmt.Println("Auto Create Log Directory : ", file_path)
		}
	}
...
func (f *fileLogger) createLogFile() (*os.File, error) {
	...
	var p int
	if f.Starttrunc == true {
		p = os.O_WRONLY | os.O_CREATE | os.O_TRUNC
	} else {
		p = os.O_WRONLY | os.O_APPEND | os.O_CREATE
	}

	fd, err := os.OpenFile(f.Filename, p, os.FileMode(perm))
	if err == nil {
...
func (f *fileLogger) createFreshFile(logTime time.Time) error {
	...
	if f.dailyOpenDate != logTime.Day() {
		for ; err == nil && num <= Max_File_Suffix_Num; num++ {
