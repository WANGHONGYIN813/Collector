...
var Local_FTP_Data_Port = 20

func Setup_Local_FTP_Data_Port(port int) {
	Local_FTP_Data_Port = port
}

func Ftp_Pasv_Mode_Init(ip string, port int) {
	newSpecificPassiveSocket(ip, port)
}

func Ftp_Server_Exit() {
	SpecificPassiveSocketExit()
}
...
func (cmd commandPasv) Execute(conn *Conn, param string) {
	...
	//socket, err := GetSpecificPassiveSocket(conn.logger, conn.sessionID)
	if err != nil {
		conn.writeMessage(425, "Data connection failed")
		return
	}
...
func (cmd commandPort) Execute(conn *Conn, param string) {
	...
	//by handawei.2019.4.17
	socket, err := newActiveSocket(host, port, conn.logger, conn.sessionID)
	//socket, err := newActiveSocketWithSpecificSrc(Local_FTP_Data_Port, host, port, conn.logger, conn.sessionID)
