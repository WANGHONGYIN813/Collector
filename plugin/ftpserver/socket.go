...
func newActiveSocketWithSpecificSrc(srcport int, remote string, port int, logger Logger, sessionID string) (DataSocket, error) {
	...
	lAddr := &net.TCPAddr{Port: srcport}
...
var pasv_socket *ftpPassiveSocket

func SpecificPassiveSocketExit() {

	if pasv_socket == nil {
		return
	}

	pasv_socket.listener_on = false

	if pasv_socket.conn != nil {
		pasv_socket.conn.Close()
	}

	if pasv_socket.listener != nil {
		pasv_socket.listener.Close()
	}
}

func newSpecificPassiveSocket(ip string, port int) error {

	pasv_socket = new(ftpPassiveSocket)
	pasv_socket.ingress = make(chan []byte)
	pasv_socket.egress = make(chan []byte)
	pasv_socket.host = ip
	pasv_socket.port = port
	pasv_socket.listener_on = true
	const retries = 10

	var err error
	for i := 1; i <= retries; i++ {
		err = pasv_socket.PassiveListenSocket()
		if err != nil && pasv_socket.port != 0 && isErrorAddressAlreadyInUse(err) {
			continue
		}
		break
	}
...
func GetSpecificPassiveSocket(logger Logger, sessionID string) (DataSocket, error) {
	pasv_socket.logger = logger
	pasv_socket.sessionID = sessionID
	return pasv_socket, nil
}

func (socket *ftpPassiveSocket) PassiveListenSocket() (err error) {

	laddr := &net.TCPAddr{Port: socket.port}

	var tcplistener *net.TCPListener
	tcplistener, err = net.ListenTCP("tcp", laddr)
	if err != nil {
		socket.logger.Print(socket.sessionID, err)
		return
	}
