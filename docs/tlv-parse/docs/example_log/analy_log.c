...
#define DS_X10_FIELD \
 ...
 field_describe g_field_describe[14][130] = {
     {
         DS_HEADER_FIELD
         DS_COM_FIELD
         DS_X10_FIELD
...
static void write_2file(FILE *fw,unsigned short data_type,unsigned short data_len,
                        char *data,unsigned char pro_type)
{
    int num = 130;
    field_describe *field = g_field_describe[pro_type];
...
static void write_tlv_info(FILE *fw,char *tlv_data,unsigned short tlv_data_len,unsigned char pro_type)
{
    char *p_tlv = tlv_data;
    ...
    while(left_len > sizeof(t_tlv_data))
    {
        ...
        write_2file(fw,data_type,data_len,tlv->value,pro_type);
...
static void get_log_info(FILE *fw,char *payload,unsigned int payload_len)
{
    ...
    write_tlv_info(fw,tlv_data,tlv_data_len,proto_type);
}
